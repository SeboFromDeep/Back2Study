...
describe('hooks', function () {
    ...
    before(async function () {
        ...
        usuario_reg = {
            username: "BuscarTareaCategoriaTestNEG",
            email: "buscartareacategoriatestNEG@gmail.com",
            password: "1234"
        };
    ...
    describe("Buscar tarea por categoria", function () {

        it("Buscar categoria que existe", async function () {
            await agent.post('/usuarios/login_user')
                .send({ correo: usuario_reg.email, password: usuario_reg.password })
                .then((res) => {
                    this.timeout(30000);
                    agent.get('/tareas/porTag/' + tareaManual.categoria)
                        .then((res) => {
        ...
        it("Buscar categoria que no existe", async function () {
            let categoria_inexistente = "@NOEXISTE";
            await agent.post('/usuarios/login_user')
                .send({ correo: usuario_reg.email, password: usuario_reg.password })
                .then((res) => {
                    this.timeout(30000);
                    agent.get('/tareas/porTag/' + categoria_inexistente)
                        .then((res) => {
                            expect(res).to.have.status(200);
                        });
                });
