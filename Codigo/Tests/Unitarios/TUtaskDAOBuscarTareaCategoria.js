...
describe('hooks', function () {
    ...
    before(async function () {
        ...
        usuario_reg = {
            username: "BuscarTareasCategoriaTestDAO",
            email: "buscartareascategoriatestDAO@gmail.com",
            password: "1234"
        };
        ...
        await dao_test.get_id_task(tarea1).then(value => {
        ...
        await dao_test.get_id_task(tarea2).then(value => {
            if (value) id_tarea2 = value;
        });
    })

    describe("Buscar tarea por categoría", function () {
        ...
        it("Debería devolver exactamente la misma lista", async function () {
            ...
            await task.buscarTareasporCategoria(categoria, id_usuario_reg).then(value => {
                expect(value[0]).to.deep.equal(prueba);
            });
        });

        it("No debería devolver nada", async function () {
            let categoria = "@INEXISTENTE";
            await task.buscarTareasporCategoria(categoria, id_usuario_reg).then(value => {
